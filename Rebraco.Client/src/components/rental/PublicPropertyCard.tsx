...
/** Property card for data fetched from the EF Core public API (not Umbraco). */
export default function PublicPropertyCard({ property }: PublicPropertyCardProps) {
  // Derive the current URL slug prefix so links stay consistent
  const location = useLocation();
  const slugPrefix = location.pathname.split('/').filter(Boolean)[0] || 'properties';
  const imgSrc = property.featuredImageUrl || null;
